...
test("Clerk - loadUsersFromFile - JSON", async () => {
  ...
  const usersWithEmail = usersFromClerk.filter(u => u.email && (Array.isArray(u.email) ? u.email.length > 0 : u.email));
  expect(usersWithEmail.length).toBeGreaterThanOrEqual(2);

  // Find users with metadata
  const usersWithMetadata = usersFromClerk.filter(u => u.publicMetadata || u.privateMetadata || u.unsafeMetadata);
  ...
  const usersWithUsernameAndPassword = usersFromClerk.filter(u => u.username && u.password && u.passwordHasher);
  expect(usersWithUsernameAndPassword.length).toBeGreaterThanOrEqual(2);

  // Find users with email and password
  const usersWithEmailAndPassword = usersFromClerk.filter(u => u.email && u.password && u.passwordHasher);
  expect(usersWithEmailAndPassword.length).toBeGreaterThanOrEqual(2);

  // Find users with phone
  const usersWithPhone = usersFromClerk.filter(u => u.phone && (Array.isArray(u.phone) ? u.phone.length > 0 : u.phone));
...
test("Auth.js - loadUsersFromFile - JSON", async () => {
  ...
  // Find users with verified emails
  const usersWithEmail = usersFromAuthjs.filter(u => u.email && (Array.isArray(u.email) ? u.email.length > 0 : u.email));
  expect(usersWithEmail.length).toBeGreaterThanOrEqual(2);
  ...
  // Find users with username and password
  const usersWithUsernameAndPassword = usersFromAuthjs.filter(u => u.username && u.password && u.passwordHasher);
  expect(usersWithUsernameAndPassword.length).toBeGreaterThanOrEqual(2);

  // Find users with email and password
  const usersWithEmailAndPassword = usersFromAuthjs.filter(u => u.email && u.password && u.passwordHasher);
  expect(usersWithEmailAndPassword.length).toBeGreaterThanOrEqual(2);

  // Find users with phone
  const usersWithPhone = usersFromAuthjs.filter(u => u.phone && (Array.isArray(u.phone) ? u.phone.length > 0 : u.phone));
  expect(usersWithPhone.length).toBeGreaterThanOrEqual(2);
});
...
test("Supabase - loadUsersFromFile - JSON", async () => {
  ...
  // Find users with verified emails
  const usersWithEmail = usersFromSupabase.filter(u => u.email && (Array.isArray(u.email) ? u.email.length > 0 : u.email));
  expect(usersWithEmail.length).toBeGreaterThanOrEqual(2);
  ...
  // Find users with username and password
  const usersWithUsernameAndPassword = usersFromSupabase.filter(u => u.username && u.password && u.passwordHasher);
  expect(usersWithUsernameAndPassword.length).toBeGreaterThanOrEqual(2);

  // Find users with email and password
  const usersWithEmailAndPassword = usersFromSupabase.filter(u => u.email && u.password && u.passwordHasher);
  expect(usersWithEmailAndPassword.length).toBeGreaterThanOrEqual(2);

  // Find users with phone
  const usersWithPhone = usersFromSupabase.filter(u => u.phone && (Array.isArray(u.phone) ? u.phone.length > 0 : u.phone));
  expect(usersWithPhone.length).toBeGreaterThanOrEqual(2);
});
...
test("Auth0 - loadUsersFromFile - JSON", async () => {
  ...
  // Find users with verified emails
  const usersWithEmail = usersFromAuth0.filter(u => u.email && (Array.isArray(u.email) ? u.email.length > 0 : u.email));
  expect(usersWithEmail.length).toBeGreaterThanOrEqual(2);
  ...
  // Find users with username and password
  const usersWithUsernameAndPassword = usersFromAuth0.filter(u => u.username && u.password && u.passwordHasher);
  expect(usersWithUsernameAndPassword.length).toBeGreaterThanOrEqual(2);

  // Find users with email and password
  const usersWithEmailAndPassword = usersFromAuth0.filter(u => u.email && u.password && u.passwordHasher);
  expect(usersWithEmailAndPassword.length).toBeGreaterThanOrEqual(2);

  // Find users with phone
  const usersWithPhone = usersFromAuth0.filter(u => u.phone && (Array.isArray(u.phone) ? u.phone.length > 0 : u.phone));
  expect(usersWithPhone.length).toBeGreaterThanOrEqual(2);
});
